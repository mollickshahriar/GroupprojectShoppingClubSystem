...
class Date //Created the class name as Date
{
private:   //Accesss specifiers used here is private so variables
	//are accesesible only in the class
	int year, month, day;

public:   // Here all constructors are defined as public as well as functions also Date is here constructor and others are functions
	Date();
	~Date();
	void setDay(int);
	int getDay();
...
Date::Date() //calling the constructor value date
{
	time_t t = time(0); //creating an object for time
	//Here we are using the current time for the location and calcultating the year value with respect to 1990
	struct tm* now = localtime(&t); // using structure to create a local time
...
{
	day = d; // using all the functionss to take tehe value fro user
}
 ...
 void beg()
{
	...
	cout<<"\t\t\tEnter product ID: ";           /*ID must be integer type */
	cin>>id;
	t->ID=id;
	cout<<"\t\t\tEnter product Name:-";           /* product name must string type*/
	cin>>name;
	t->proName=name;
	cout<<"\t\t\tEnter product price at $: ";    /* price must be integer or double type */
	cin>>pre;
	t->prePrice=pre;
	cout<<"\t\t\tEnter product quantity: ";   /* quantity must be integer type */
	cin>>quant;
	t->quantity=quant;
	if(head==NULL)
	{
...
///////////////////////////////////////////////////////////////////////
	void buy()
	{
      ...
      {              //    show that item you have bought
       ...
       double price=pay*(0.90);           //    with 10% discount
	    cout<<"\n\nOriginal price : "<<pay;
        cout<<"\n with 10% discount: "<<(double)price<<"\nThank you! for the shopping\n\n";
...
int main()
{
	...
	cout<<"--------------------------------------------------"<<endl;
   	gotoxy(17,11);
   	cout<<"|Subject Teacher ->> BIN CAO <<-- |\n"<<endl;
   	gotoxy(17,13);
   	cout<<">>>----Project Implemented By-----<<<"<<endl;
    gotoxy(22,15);
   	cout<<"Abdul Rahman Sesay (L201926100111)"<<endl;
   	gotoxy(22,16);
   	cout<<"Mollick Shahriar Hossain  (L201826100121)"<<endl;
   	gotoxy(22,17);
   	cout<<"Isfaoui Oussama   (L201926100116)"<<endl;
   	gotoxy(22,18);
   	cout<<"Amina Mouchtahi   (L201926100130)"<<endl<<endl;
